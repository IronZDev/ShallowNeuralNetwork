...
column3 = [
    ...
    [sg.Text('Number of hidden layers')],
    [sg.Text('Number of neurons in each hidden layer')]
]
...
column4 = [
    ...
    [sg.Spin([i for i in range(1, 5)], initial_value=2, key='hiddenLayersNum', size=(1, 1))],
    [sg.Spin([i for i in range(1, 8)], initial_value=4, key='neuronsInLayer', size=(1, 1))],
]
layout = [
...
# Create the Window
window = sg.Window('Neural Network', layout).Finalize()
        ...
        if is_valid():
            ...
            for i in range(len(points[0])):
                ...
                if points[2][i] is 'r':
                    ...
                    outputs.append(1)
            network = Network(values['activation'].lower().replace(' ', '_'),
                            float(values['learningRate']),
                            int(values['hiddenLayersNum']),
                            int(values['neuronsInLayer']))
            network.train(np.asarray(inputs), np.asarray(outputs), int(values['epochs']))

            if 'fig_canvas_agg' in globals():  # Update if plot already exists
                plt.clf()
                plt.scatter(points[0], points[1], c=points[2])
                draw_decision_surface(network, np.amin(points[0]), np.amax(points[0]), np.amin(points[1]),
                                      np.amax(points[1]))
                fig_canvas_agg.draw()
